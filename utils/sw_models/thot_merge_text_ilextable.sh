...
# Merges a set of lexical counts generated during the estimation of an
...
merge_sorted_counts()
{
    ${AWK} -v n_val=${n_val} '\
     ...
     function print_counts()
     {
       ...
       for(n=2;n<=num_trg_words;++n)
       {
         if(first_trg==trgword[n])
         {
...
if [ $# -ne 2 -a $# -ne 3 -a $# -ne 4 -a $# -ne 5 ]; then
    echo "Usage: thot_merge_text_ilextable [-t <text_ilextable>] [-ns] [-T <tmpdir>]"
else
    # Take parameters
    t_given=0
    ttable_file=""
